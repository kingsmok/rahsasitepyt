...
خزش نقش‌محور جدید (`scripts/commercial_role_audit.py`) یک دیتابیس موقت کامل می‌سازد و لینک‌ها، action/method فرم‌ها و منابع واقعی HTML را بررسی می‌کند. در این مرحله مواردی که تست‌های قدیمی ندیده بودند برطرف شد:
...
- ویجت فروش دیگر قیمت رقیب یا deadline شمارش‌معکوس ساختگی تولید نمی‌کند؛ فقط تخفیف و مقایسه‌ای که واقعاً در دیتابیس ثبت شده نمایش داده می‌شود.
- محیطی که داده seed توسعه را فعال کرده، نوار واضح «نسخه نمایشی» دارد و فعال‌سازی دمو در production حتی با env صریح رد می‌شود.
- نصب واقعی `/install/run` روی SQLite خالی از طریق HTTP تا پایان اجرا شد: فقط یک سوپرادمین، صفر دوره، سایت غیرفعال و بدون حساب/محتوای نمونه.
