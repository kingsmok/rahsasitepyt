(function(){'use strict';function csrfToken(){var m=document.querySelector('meta[name="csrf-token"]');if(m&&m.content)return m.content;var h=document.getElementById('global-csrf-token');return(h&&h.value)||'';}window.csrfToken=csrfToken;function toast(msg,type){var box=document.querySelector('.toasts');if(!box){box=document.createElement('div');box.className='toasts';box.setAttribute('role','region');box.setAttribute('aria-label','اعلان‌ها');document.body.appendChild(box);}
...
if(d.ok){if(d.fav){btn.classList.add('active');if(btn.querySelector('span'))btn.querySelector('span').textContent='❤️ ذخیره شد';toast(d.msg,'success');}
else{btn.classList.remove('active');var card=btn.closest('.course-card')||btn.closest('[data-fav-card]');if(card&&card.hasAttribute('data-fav-card')){card.style.transition='opacity .3s';card.style.opacity='0';setTimeout(function(){card.remove();},300);}toast(d.msg,'info');}
...
document.querySelectorAll('.poster').forEach(function(poster){poster.addEventListener('click',function(){var video=poster.parentElement.querySelector('video');if(video){poster.style.display='none';video.play().catch(function(){});}});});document.querySelectorAll('.img-picker').forEach(function(img){img.addEventListener('click',function(){var input=document.getElementById('image-input');if(input){input.value=img.dataset.src;document.querySelectorAll('.img-picker').forEach(function(i){i.style.outline=i===img?'3px solid var(--primary)':'none';});var prev=document.getElementById('course-image-preview');if(prev){prev.src=img.src;}}});});function copyText(value){if(navigator.clipboard&&window.isSecureContext){return navigator.clipboard.writeText(value);}return new Promise(function(resolve,reject){var area=document.createElement('textarea');area.value=value;area.setAttribute('readonly','');area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();try{document.execCommand('copy');resolve();}catch(err){reject(err);}finally{area.remove();}});}window.copyText=copyText;document.querySelectorAll('[data-copy]').forEach(function(btn){btn.addEventListener('click',function(){var val=btn.dataset.copy||'';copyText(val).then(function(){toast('در کلیپ‌بورد کپی شد','success');}).catch(function(){toast('کپی خودکار ممکن نشد؛ متن را دستی کپی کنید.','error');});});});var btt=document.getElementById('back-to-top');if(btt){window.addEventListener('scroll',function(){btt.classList.toggle('show',window.scrollY>400);},{passive:true});btt.addEventListener('click',function(){window.scrollTo({top:0,behavior:'smooth'});});}
...
return null;}
document.querySelectorAll('.pb-share-btn').forEach(function(btn){btn.addEventListener('click',function(){var net=btn.dataset.share;var url=shareLink(net);if(net==='copy'){copyText(location.href).then(function(){toast('لینک کپی شد ✅','success');}).catch(function(){toast('کپی لینک ممکن نشد','error');});}else if(url){window.open(url,'_blank','width=640,height=520');}});});document.querySelectorAll('.pb-code-num code').forEach(function(code){var lines=(code.textContent||'').split('\n');code.innerHTML=lines.map(function(l){return'<span>'+l+'</span>';}).join('');});document.querySelectorAll('.pb-aw-words').forEach(function(wrap){var words=[];try{words=JSON.parse(wrap.dataset.words||'[]');}catch(e){}
...
fetch('/api/form',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':(document.querySelector('meta[name="csrf-token"]')||{}).content||''},body:JSON.stringify({fields:fields,to:form.dataset.to||'contact'})}).then(function(r){return r.json();}).then(function(d){if(btn){btn.disabled=false;btn.textContent=form.querySelector('button[type=submit]').dataset.orig||'ارسال';}
